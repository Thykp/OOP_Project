  ...
  const handleInputChange = (field: string, value: string | boolean) => {
    ...
    if (field === "password") {
      setPasswordValidation({
        length: value.toString().length >= 8,
        uppercase: /[A-Z]/.test(value.toString()),
        lowercase: /[a-z]/.test(value.toString()),
        number: /\d/.test(value.toString()),
        special: /[!@#$%^&*(),.?":{}|<>]/.test(value.toString()),
      })
    }
  }
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {formData.password && (
                    <div className="mt-2 space-y-1">
                      ...
                      <div className="grid grid-cols-2 gap-1 text-xs">
                        <div
                          className={`flex items-center space-x-1 ${passwordValidation.length ? "text-green-600" : "text-gray-400"}`}
                        >
                          {passwordValidation.length ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                          <span>8+ characters</span>
                        </div>
                        <div
                          className={`flex items-center space-x-1 ${passwordValidation.uppercase ? "text-green-600" : "text-gray-400"}`}
                        >
                          {passwordValidation.uppercase ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                          <span>Uppercase letter</span>
                        </div>
                        <div
                          className={`flex items-center space-x-1 ${passwordValidation.lowercase ? "text-green-600" : "text-gray-400"}`}
                        >
                          {passwordValidation.lowercase ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                          <span>Lowercase letter</span>
                        </div>
                        <div
                          className={`flex items-center space-x-1 ${passwordValidation.number ? "text-green-600" : "text-gray-400"}`}
                        >
                          {passwordValidation.number ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}
                          <span>Number</span>
                        </div>
                        <div
                          className={`flex items-center space-x-1 col-span-2 ${passwordValidation.special ? "text-green-600" : "text-gray-400"}`}
                        >
                ...
                <div className="space-y-2">
                  ...
                  {formData.confirmPassword && (
                    <div
                      className={`text-xs flex items-center space-x-1 ${passwordsMatch ? "text-green-600" : "text-red-500"}`}
                    >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    <Label htmlFor="marketing" className="text-sm text-gray-600 leading-relaxed">
                      I would like to receive appointment reminders, health tips, and promotional emails from SingHealth
                      Clinic
                    </Label>
                  </div>
                </div>
