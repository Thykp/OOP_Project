...
export default function StaffDashboard() {
  ...
  const loadDoctorsIntoCaches = (arr: any[]) => {
    try {
      arr.forEach((d: any) => {
        ...
        if (cid && cname) clinicCache.current.set(String(cid), cname)
      })
  ...
  function normalizeDate(val: any) {
    ...
    if (typeof val === "string" && val.length >= 10) {
      return val.slice(0, 10); // handles "YYYY-MM-DD..." string
    }
    return "";
  }
  ...
  function normalizeTime(val: any) {
    ...
    if (typeof val === "string" && val.length >= 4) {
      return val.slice(0, 5); // handles "HH:MM" or full ISO time
    }
    return "";
  }
  ...
  useEffect(() => {
    ...
    const statusSub = subscribeToAppointmentStatus(async (update: any) => {
      try {
        ...
        // CANCELLED / NO_SHOW -> remove from lists and notify
        if (status === "CANCELLED" || status === "NO_SHOW") {
          setAppointments(prev => prev.filter(a => a.appointment_id !== apptId))
        ...
        // RESCHEDULED / SCHEDULED -> upsert in-memory (no full fetch)
        if (status === "RESCHEDULED" || status === "SCHEDULED") {
        ...
        if (status === "CHECKED-IN" || status === "CHECKED IN") {
          ...
          // update queueAppointments if payload contains queue info
          if (update.queueNumber || update.position) {
            setQueueAppointments(prev => {
              const item: QueueItem = {
                ...buildFromEvent(update),
                queueNumber: Number(update.queueNumber ?? update.queue_number ?? 0),
                position: Number(update.position ?? update.pos ?? 0),
                isFastTrack: Number(update.position ?? update.pos ?? 0) === 1 && (update.queueLength ? Number(update.queueLength) > 1 : true),
              }
        ...
        // For any other statuses, attempt to update status in-place only
        ...
        console.error("[StaffDashboard] appointment status handler error:", err)
      }
    })

    return () => {
      try { if (statusSub && typeof statusSub.unsubscribe === "function") statusSub.unsubscribe() } catch (e) { /* ignore */ }
      // keep global socket connection if other hooks/pages rely on it
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [staffClinicId])
              ...
              <Card className="mb-4">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                      <Label className="text-sm">Patient Name</Label>                      <Input
                        type="text"
