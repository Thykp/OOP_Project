...
export default function AdminClinicConfig() {
  ...
  // Fetch doctors on mount only
  useEffect(() => {
    fetchDoctors()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  // Fetch time slots only when a doctor is selected
  useEffect(() => {
    if (selectedDoctor && selectedDoctor !== "ALL") {
      fetchTimeSlots(selectedDoctor)
    } else {
      // Clear time slots when no doctor is selected
      setTimeSlots([])
      setLoading(false)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleCreateBulkSlots = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `Created ${created.length} time slots successfully`
      })

      // Refresh slots for selected doctor
      if (selectedDoctor) {
        fetchTimeSlots(selectedDoctor)
      }
  ...
  const handleDeleteSlot = async () => {
    ...
    try {
      const response = await fetch(`${baseURL}/api/timeslots/admin/${slotToDelete.id}`, {
        method: "DELETE"
      })
  ...
  const filteredSlots = (timeSlots || []).filter(slot => 
    filterDay === "ALL" || (slot.dayOfWeek && slot.dayOfWeek === filterDay)
  )
  ...
  return (
    <PageLayout>
      <div className="container mx-auto max-w-7xl px-4 py-8">
        ...
        {/* Doctor Selection - Required First Step */}
        <Card className="mb-6">
          <CardHeader>
          ...
          <CardContent>
            <div className="max-w-md">
              <Label htmlFor="doctor-filter">Select Doctor *</Label>
        ...
        {/* Create/Edit Form - Only shown when doctor is selected */}
        {selectedDoctor && (
          <Card className="mb-6">
            <CardHeader>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                  ...
                  <Select
                    value={formData.doctorId || selectedDoctor}
                    onValueChange={(value) => {
                      ...
                      // Update selected doctor when changed in form
        ...
        {/* Time Slots List - Only shown when doctor is selected */}
        {!selectedDoctor ? (
          <Card>
            <CardHeader>
              <CardTitle>Step 2: View Time Slots</CardTitle>
              <CardDescription>
                Please select a doctor above to view and manage their time slots
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-center py-12 text-gray-500">
                <Calendar className="h-16 w-16 mx-auto mb-4 text-gray-400" />
                <p className="text-lg font-medium mb-2">No Doctor Selected</p>
                <p>Select a doctor from the dropdown above to get started</p>
              </div>
            </CardContent>
          </Card>
        ) : (
          <Card>
            <CardHeader>
              <CardTitle>Step 2: Time Slots</CardTitle>
              <CardDescription>
                {filteredSlots.length} slot(s) found
                {selectedDoctor && ` for selected doctor`}
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-600">
                            {slot.dayOfWeek ? (slot.dayOfWeek.charAt(0) + slot.dayOfWeek.slice(1).toLowerCase()) : "Unknown Day"} • {formatTime(slot.startTime)} - {formatTime(slot.endTime)}
                          </p>
                        </div>
                      </div>
              ...
              <AlertDialogDescription>
                ...
                {slotToDelete && (
                  <div className="mt-2 p-2 bg-gray-100 rounded">
                    ...
                    {slotToDelete.dayOfWeek ? (slotToDelete.dayOfWeek.charAt(0) + slotToDelete.dayOfWeek.slice(1).toLowerCase()) : "Unknown Day"} • {formatTime(slotToDelete.startTime || "")} - {formatTime(slotToDelete.endTime || "")}
                  </div>
                )}
              </AlertDialogDescription>
            </AlertDialogHeader>
